    ...
    for key in sorted(raw_diff.keys()):
        ...
        try:
            if type(data['value']) == dict:
                ...
                styled_diff.append('{}{}{}: {}'.format(indent, BASE_INDENT, key, formatted_value))
                continue
        except (TypeError, KeyError):
            pass
        if data['status'] == 'changed':
            formatted_value_before = represent(data['value'][0], level=level + 1)
            formatted_value_after = represent(data['value'][1], level=level + 1)
            styled_diff.append('{}{}{}: {}'.format(indent, INDENT_IF_DELETED, key, formatted_value_before))
            styled_diff.append('{}{}{}: {}'.format(indent, INDENT_IF_ADDED, key, formatted_value_after))
            continue
        formatted_value = represent(data['value'], level=level + 1)
        if data['status'] == 'deleted':
            styled_diff.append('{}{}{}: {}'.format(indent, INDENT_IF_DELETED, key, formatted_value))
        elif data['status'] == 'added':
            styled_diff.append('{}{}{}: {}'.format(indent, INDENT_IF_ADDED, key, formatted_value))
...
def represent(data, level):
    ...
    for key in data:
        if type(data[key]) != dict:
            result.append('{}{}{}: {}'.format(indent, BASE_INDENT, key, data[key]))
        else:
            value = represent(data[key], level=level+1)
            result.append('{}{}{}: {}'.format(indent, BASE_INDENT, key, value))
